...
const pedirlo_btn = document.querySelector("#pedirlo-btn");
const momo_container = document.querySelector("#momo");
...
function loadMomo() {
    return fetch("https://meme-api.com/gimme")
        .then((res) => res.json())
        .then((json) => {
            momo_container.src = json.url;
        });
}

pedirlo_btn.addEventListener("click", () => {
    Swal.fire({
        title: "Seguro que quieres solicitar un MOMARDIUM",
        showDenyButton: true,
        ...
        if (result.isConfirmed) {
            loadMomo().then(() => {
                momo_container.classList.remove("hidden");
            });
        } else if (result.isDenied) {
            Swal.fire("Bueno :(", "", "error");
        }
    });
});
